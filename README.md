...
Version 0.3.1 is a focused patch release that adds R-compatible
median/MAD winsorization for row-wise omics transforms.
...
- Prebuilt v0.3.1 artifacts and checksums are on the
  [GitHub release page](https://github.com/robbyjo/JLinAlg/releases/tag/v0.3.1).
  Use `jlinalg-0.3.1.jar` for the self-contained CLI; the separate
  `JLinAlg-0.3.1-library.jar` is the thin library.
- A JDK 17 or newer.
...
```powershell
java -jar build/cli/jlinalg-0.3.1.jar `
...
```powershell
java -jar build/cli/jlinalg-0.3.1.jar beta-regression `
...
```powershell
java -jar build/cli/jlinalg-0.3.1.jar penalized-regression `
