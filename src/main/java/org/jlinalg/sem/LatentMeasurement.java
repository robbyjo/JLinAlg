...
/** Deterministic principal-factor measurement model for continuous indicators. */
public final class LatentMeasurement {
    private LatentMeasurement() { }

    /** Extracts a bounded number of latent factors from complete indicators. */
    public static Result fit(double[][] data, int factorCount, int maximumIterations,
        ...
        for (int factor = 0; factor < factorCount; factor++) {
            double[] vector = new double[indicators]; for (int column = 0; column < indicators; column++) vector[column] = 1.0 / Math.sqrt(indicators);
