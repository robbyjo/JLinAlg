...
tab <- function(path) read.delim(path,check.names=FALSE,stringsAsFactors=FALSE,quote='"',comment.char="")
write <- function(x,path) write.table(x,path,sep="\t",row.names=FALSE,quote=FALSE,na="NA")
if(mode == "state") {
  if(!requireNamespace("limma",quietly=TRUE)) stop("Install limma in the chosen R library before using state")
  raw <- tab("counts.tsv"); counts <- as.matrix(raw[,-1,drop=FALSE]); rownames(counts) <- raw[[1]]
  ...
  set.seed(as.integer(args[3])); raw <- tab("normalized.tsv"); x <- as.matrix(raw[,-1,drop=FALSE])
