...
fit_gene <- function(feature) {
  formula <- as.formula(paste(
    "BMI ~ s(", feature, ", bs='ps', k=10, m=2) +",
    fixed_rhs, "+ s(Batch, bs='re')"
  ))
...
for (measurement in seq_len(options$measurements)) {
  ...
  timing_rows[[measurement]] <- data.table(
    runtime = "R", model = "batch-gamm", backend = "mgcv",
  ...
  if (measurement == 1L) {
    result_rows <- rbindlist(lapply(seq_along(fits), function(index) {
      ...
      variance <- fit$sig2 / fit$sp
      data.table(
        runtime = "R", model = "batch-gamm",
        feature_key = features$feature_key[[index]],
        feature_id = features$feature_id[[index]],
        edf = unname(smooth_table[1L, "edf"]),
        ...
        pedigree_variance = NA_real_,
        batch_variance = unname(variance[[2L]]),
        smooth_variance = unname(variance[[1L]]),
        residual_variance = unname(fit$sig2)
      )
    }))
  }
}
...
fwrite(timings,
  file.path(options$prepared_dir, "r_batch_gamm_timings.csv"))
fwrite(result_rows,
  file.path(options$prepared_dir, "r_batch_gamm_results.csv"))
print(timings)
