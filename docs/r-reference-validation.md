...
The [September 8 statistical audit](advanced-validation.md) adds joint
latent/mean/ordinal/FIML SEM, metafor/clubSandwich multilevel inference,
lme4/lmerTest/pbkrtest sparse mixed profiles, exact diffuse and missing-series
ARIMA, conditional MR, adaptive GLMM integration, and regression/selection
fixtures. Their generators also live under `src/test/R`,
`src/test/resources/meta`, and `src/test/resources/timeseries`. Family reports
record the exact R packages, statistical definitions, tolerances, and timings.
The older reference inventory below remains useful for the earlier engines.
