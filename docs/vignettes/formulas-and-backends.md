...
```bash
java -jar jlinalg-0.3.6.jar --pheno phenotype.csv --omics expression.csv --id SampleName --formula "BMI ~ Sex * treatment + Age + <omics> + (1|site)" --backend preferred --threads 96 --block-size auto --out bmi-model.csv --dry-run

java -jar jlinalg-0.3.6.jar --pheno phenotype.csv --omics expression.csv --id SampleName --formula "BMI ~ Sex * treatment + Age + <omics> + (1|site)" --backend cpu --threads 96 --block-size auto --out bmi-model.csv --overwrite
```
