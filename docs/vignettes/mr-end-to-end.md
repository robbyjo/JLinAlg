...
This multiple-exposure/single-outcome model is now exposed as `mr-mvmr`.
Do not call it multivariate outcome MR. For a related family of two or more
outcomes, `mr-multivariate` fits covariance-aware summary-data multivariate
MR-IVW and an optional Mahalanobis/bootstrap multivariate MR-PRESSO diagnostic.
The [joint-model MR vignette](multivariate-mr.md) gives the complete wide input,
outcome-correlation matrix, commands, joint tests, and method boundaries.
