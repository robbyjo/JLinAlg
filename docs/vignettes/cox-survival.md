...
```bash
java -jar jlinalg-0.3.6.jar --pheno phenotype.csv --id SampleName --formula "Surv(followup,event) ~ Sex + Age" --ties efron --out cox.csv

java -jar jlinalg-0.3.6.jar --pheno phenotype.csv --id SampleName --individual-id IID --formula "Surv(followup,event) ~ Sex + Age" --grm cohort --ties efron --out cox-grm.csv
```
